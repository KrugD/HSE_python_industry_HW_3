...
from app import models, schemas, crud, cache, utils
...
def create_short_link(
    ...
    current_user: models.User = Depends(get_current_user, use_cache=False)  # Опциональная аутентификация
):
    """
...
def update_link(
    ...
    link: schemas.LinkCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)  # Обязательная аутентификация
):
    """
...
def delete_link(
    short_code: str,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)  # Только для авторизованных
):
    """
...
def delete_all_links(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)  # Только для админов
):
    """
